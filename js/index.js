...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (slider && prevBtn && nextBtn && indicators.length) {
    ...
    slider.addEventListener("touchend", (e) => {
      ...
      // If moved significantly, prevent click
      if (Math.abs(delta) > 10) {
        e.preventDefault(); // This might be too aggressive, but let's see logic below
      }
    ...
    // Prevent link clicks if dragging happened (better logic)
    ...
    links.forEach(link => {
      link.addEventListener('click', (e) => {
        // In a simple CSS slider, click usually works. 
        // If we had complex JS drag, we'd need this.
        // Currently the JS handles touch but not mouse drag.
        // The issue is likely simpler: maybe the filter logic in catalog.js matches exactly "brand"
        // But the value is "New%20Era" -> "New Era".
        // Ensure catalog.js decodes URI components.
      });
    });
